...
class OpenAICompatibleProvider(VLMProvider):
    def __init__(self, model: str, base_url: str, api_key: str) -> None:
        if not (model and base_url and api_key): raise ValueError("VLM_MODEL, VLM_BASE_URL, and VLM_API_KEY are required")
    ...
    async def analyze(self, images: list[Any], observation: dict[str, Any], sop_context: dict[str, Any]) -> VLMResponse:
        ...
        def request() -> str:
            raw = json.dumps(payload).encode(); req = Request(f"{self.base_url}/chat/completions", raw, {"Authorization":f"Bearer {self.api_key}", "Content-Type":"application/json"})
